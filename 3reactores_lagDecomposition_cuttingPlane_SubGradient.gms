$TITLE 3reactores, descomposicion lagrangiana con cutting plane y subgradiente
$ontext
fecha: 24/02/2021
autor: Daniel Montes
comentarios: cutting plane con sets dinámicos y alargamiento/encogomiento
             de la región fáctible de los multiplicadores, se añaden dos
             restricciones que integran el método del subgradiente al
             del cutting plane
problemas: Converge a un punto no factible del problema global. Se violan las
           restricciones de recursos compartidos. La violación es mayor que
           cuando se aplica el mismo algoritmo híbrido al ejercicio con
...
Parameters
    ...
    lambda_1(i) "Multiplicadores de reactivos"      /1 10, 2 10, 3 10/
    lambda_2(i) "Multiplicadores de refrigerante"   /1 1, 2 1, 3 1/
    subgradiente(j,iter)
    theta_k(j,iter)
    dual_value(iter)
    subgradiente_K(j)
...
multipliersL(j).. theta(j) =G= lambda_K(j) - step_size*(UB_K - dual_value_K)*abs(subgradiente_K(j))/sqr(norma2_K);
multipliersU(j).. theta(j) =L= lambda_K(j) + step_size*(UB_K - dual_value_K)*abs(subgradiente_K(j))/sqr(norma2_K);
...
model update_multipliers /Objz, trust_region, multipliersL, multipliersU/;
...
* Se activa una restricción adicial del cutting plane
    ...
    lambda_K(j) = theta_k(j, iter);
    norma2_K = sqrt( sum(j,  sqr(subgradiente(j, iter))) );
    subgradiente_K(j) = subgradiente(j, iter);
    dual_value_K = J_dual;
    UB_K = J_upper;
...
******************************************************
    if (sum(j, sqr(theta_k(j, iter) - theta_k(j, iter-1))) <= 1e-10,
        break
        );
...
*    put theta_k('m1', iter), theta_k('m2', iter), theta_k('m3', iter), theta_k('m4', iter), theta_k('m5', iter) ,theta_k('m6', iter) /;
