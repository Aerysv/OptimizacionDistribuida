...
Parameters
    ...
    alpha(i)   "Parámetros de transmisión de calor (kJ/ºC)"
                /1 1,
                 2 1.22,
                 3 1.42/
    qfix(i)    "Caudal de reactivos (L/min)"
            /1 1,
             2 3,
             3 2/
    qcfix(i)   "Caudal de refrigerante (L/min)"
            /1 14,
             2 25,
             3 10/
...
Scalars
    ...
    c_qc    "Precio del refrigerante (€/L)" /0.1/

    u_1    "Multiplizador del caudal de reactivos" /10/
    u_2    "Multiplizador del caudal de refrigerante" /10/
    B_1    /0.25/
    B_2     /0.025/
...
CostoIndividual1..  J1 =E= -(q('1')*(c_b*Cb('1') + c_c*Cc('1') + c_d*Cd('1') - c_a*Ca('1')) - c_qc*qc('1'))
                           + 1/(2*B_1)*( sqr( max(0, u_1 + B_1*(q('1') + qfix('2') + qfix('3') - qmax ) ) ) - sqr(u_1) )
                           + 1/(2*B_2)*( sqr( max(0, u_2 + B_2*(qc('1') + qcfix('2') + qcfix('3') - qcmax ) ) ) - sqr(u_2) )
                           + Penalizacion*(sqr(q('1') - qfix('1'))+sqr(qc('1') - qcfix('1'))/10);
...
CostoIndividual2..  J2 =E= -(q('2')*(c_b*Cb('2') + c_c*Cc('2') + c_d*Cd('2') - c_a*Ca('2')) - c_qc*qc('2'))
                           + 1/(2*B_1)*( sqr( max(0, u_1 + B_1*(qfix('1') + q('2') + qfix('3') - qmax ) ) ) - sqr(u_1) )
                           + 1/(2*B_2)*( sqr( max(0, u_2 + B_2*(qcfix('1') + qc('2') + qcfix('3') - qcmax ) ) ) - sqr(u_2) )
                           + Penalizacion*(sqr(q('2') - qfix('2'))+sqr(qc('2') - qcfix('2'))/10);
...
CostoIndividual3..  J3 =E= -(q('3')*(c_b*Cb('3') + c_c*Cc('3') + c_d*Cd('3') - c_a*Ca('3')) - c_qc*qc('3'))
                           + 1/(2*B_1)*( sqr( max(0, u_1 + B_1*(qfix('1') + qfix('2') + q('3') - qmax ) ) ) - sqr(u_1) )
                           + 1/(2*B_2)*( sqr( max(0, u_2 + B_2*(qcfix('1') + qcfix('2') + qc('3') - qcmax ) ) ) - sqr(u_2) )
                           + Penalizacion*(sqr(q('3') - qfix('3'))+sqr(qc('3') - qcfix('3'))/10);
...
*Subproblemas
    solve subproblema1 minimizing J1 using DNLP;
    solve subproblema2 minimizing J2 using DNLP;
    solve subproblema3 minimizing J3 using DNLP;
...
*Master
    ...
    u_1 = u_1 + B_1*subgradiente1;
    u_2 = u_2 + B_2*subgradiente2;
        
    if (norma2 < norma2_min,
        indice = n_iter;
        norma2_min = norma2;
        );
    J = sum(i, -(q.l(i)*(c_b*Cb.l(i) + c_c*Cc.l(i) + c_d*Cd.l(i) - c_a*Ca.l(i)) - c_qc*qc.l(i)));
    
    J_dual = J1.l + J2.l + J3.l - 
    put n_iter, q.l('1'), qc.l('1'), u_1, u_2, subgradiente1, subgradiente2, norma2, J, indice /;
...
putclose;
Display q.l, qc.l, T.l, Tc.l, Ca.l, Cb.l, J1.l, J2.l, J3.l;
