...
******************************************************
*** Calculate the dual value (Lower Bound)
******************************************************
...
* o en la cota superior.
    ...
    if (qmax - q.l('2') - q.l('3') > 0,
        q.l('1') = qmax - q.l('2') - q.l('3');
    elseif qmax - q.l('1') - q.l('3') > 0,
        q.l('2') = qmax - q.l('1') - q.l('3');
    elseif qmax - q.l('1') - q.l('2') > 0,
        q.l('3') = qmax - q.l('1') - q.l('2');
        );
    if (qcmax - qc.l('2') - qc.l('3') > 0,
        qc.l('1') = qcmax - qc.l('2') - qc.l('3');
    elseif qcmax - qc.l('1') - qc.l('3') > 0,
        qc.l('2') = qcmax - qc.l('1') - qc.l('3');
    elseif qcmax - qc.l('1') - qc.l('2') > 0,
        qc.l('3') = qcmax - qc.l('1') - qc.l('2');
        );

    q.fx(i) = q.l(i);
    qc.fx(i) = qc.l(i);
    if (sum(i, q.l(i)) + sum(i, qc.l(i)) <= qmax + qcmax,
        solve global minimizing J_total using NLP;
        );
*Se debe elegir el minímo de J_up como cota superior
    if (J_total.l < J_up,
        J_up = J_total.l;
        );
...
* Se activa una restricción adicial del cutting plane
    ...
    norma2_K = sqrt( sqr(subgradiente('m1', iter)) + sqr(subgradiente('m2', iter)));
    
    theta_k('m1', iter) = lambda_1;
    theta_k('m2', iter) = lambda_2;
    
    lambda_K('m1') = lambda_1;
    lambda_K('m2') = lambda_2;
...
******************************************************
    if (sqr(sum(j, theta_k(j, iter) - theta_k(j, iter-1))) <= 1e-6,
        break
        );

* Guardar fichero de datos  
    put n_iter, q.l('1'), q.l('2'), q.l('3'), qc.l('1'), qc.l('2'), qc.l('3'), lambda_1, lambda_2, J_dual, UB_K /;
    );    
